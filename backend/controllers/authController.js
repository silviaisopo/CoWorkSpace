...
exports.register = async (req, res) => {
  try {
    ...
    if (!name || !email || !password) return res.status(400).json({ success: false, message: 'Dati mancanti' });
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) return res.status(401).json({ success: false, message: 'Email o password errati' });
