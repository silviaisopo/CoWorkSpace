...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 🔹 Carica prenotazioni già fatte dal backend
    async function loadBookings(date) {
        try {
            ...
            // 🔹 se vuoi filtrare lato frontend per la data scelta
            return allBookings.filter(b => {
                const bookingDate = new Date(b.start_time).toISOString().split("T")[0];
                return bookingDate === date.split("/").reverse().join("-");
            });
    ...
    async function renderDailySlots() {
        ...
        for (let h = 8; h < 20; h++) {
            ...
            // Controllo se passato (solo se è oggi)
            const isPast = isToday && h <= currentHour;
            ...
            if (isBooked) {
                // 🔴 già prenotato
                btn.className = "slot-button p-3 text-sm rounded-lg w-full bg-red-500 text-white cursor-not-allowed";
            } else if (isPast) {
                // ⚪ libero ma passato
                btn.className = "slot-button p-3 text-sm rounded-lg w-full bg-gray-300 text-white cursor-not-allowed";
            } else {
                // 🟢 libero e prenotabile
                btn.className = "slot-button p-3 text-sm rounded-lg w-full bg-green-200 hover:bg-green-300";
                btn.addEventListener("click", () => toggleSlot(slot, btn));
            }
    ...
    // 🔹 Conferma prenotazione → controlla disponibilità dal backend
    confirmButton.addEventListener("click", async () => {
        ...
        // 🔹 Controllo disponibilità lato backend
        const res = await fetch("/api/bookings/check-availability", {
            ...
            body: JSON.stringify({ location_id: location.id, start_time, end_time })
        });

        const result = await res.json();
        if (!result.available) {
            alert("Slot non più disponibile, scegli un altro orario.");
            await renderDailySlots();
            return;
        }

        // 🔹 Salvo la prenotazione temporanea in localStorage
        const user = JSON.parse(localStorage.getItem("user")) || {};
        const totalPrice = parseFloat(location.price_per_hour) || 0;
        const bookingData = {
            user_id: user.id,
