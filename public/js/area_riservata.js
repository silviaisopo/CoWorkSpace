...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (deleteAccountBtn) {
        deleteAccountBtn.addEventListener("click", async () => {
            if (!confirm("Sei sicuro di voler eliminare il tuo account? Questa operazione è irreversibile.")) return;
    ...
    // Carica prenotazioni e statistiche
    loadUserStats();
    loadBookings();
});

// Funzione per caricare statistiche prenotazioni
async function loadUserStats() {
    const token = localStorage.getItem("token");
    if (!token) return;
    ...
    try {
        ...
        const nextBooking = activeBookings.sort((a, b) => new Date(a.start_time) - new Date(b.start_time))[0];
        document.querySelector("#dashboard-next").textContent = nextBooking ? formatDate(nextBooking.start_time) : "Nessuna";
...
// Funzione per caricare prenotazioni
async function loadBookings() {
    const token = localStorage.getItem("token");
    if (!token) return;
    ...
    bookings.forEach(booking => {
        ...
        bookingCard.innerHTML = `
            ...
            <div class="flex justify-between items-center border-t pt-4">
                <div>
                    <p class="text-sm text-gray-600">Prezzo totale</p>
                    <p class="font-bold text-[#4a3729]">€${parseFloat(booking.total_price).toFixed(2)}</p>
                </div>
            </div>
        `;
        container.appendChild(bookingCard);
